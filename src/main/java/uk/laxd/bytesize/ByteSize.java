 ...
 *
 * <p>All methods that take a {@code size} without a corresponding {@link ByteUnit} are
...
public class ByteSize {
     ...
     *
     * <p>Formatting is done by repeatedly converting from the source {@link ByteUnit}
     * to increasingly larger ones until a filesize is found that can be represented using only
     * that {@link ByteUnit}, and moving up to the next one would result in less
     ...
     * @return The formatted value, represented as multiples of the largest unit such that the next largest unit
     * would have to be represented solely as a fraction.
     * @see #getFormattedFileSize(BigDecimal, ByteUnit, int)
     */
    ...
    public String getFormattedFileSize(BigDecimal size, int scale) {
        return getFormattedFileSize(size, ByteUnit.BYTE, scale);
    }
    ...
    public String getFormattedFileSize(BigDecimal size) {
        return getFormattedFileSize(size, ByteUnit.BYTE, DEFAULT_SCALE);
    }
